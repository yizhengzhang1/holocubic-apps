-- Device-only capture probe for the Comet gallery GIF.
-- Captures 16 native 320x240 PNG frames from launch through the long-tail burst.

local previous = rawget(_G, "FIREWORKS_COMET_GIF_CAPTURE_PROBE")
if previous and previous.stop then pcall(previous.stop, "reload") end

local OUTPUT_DIR = "/sd/fireworks-comet-gif"
local REPORT_PATH = "/sd/fireworks-comet-gif-report.json"
...
_G.FIREWORKS_COMET_GIF_CAPTURE_PROBE = PROBE
...
function PROBE.stop()
  ...
  if PROBE.target and PROBE.target.stop then
    pcall(PROBE.target.stop, "comet-gif-capture-probe")
  end
  if rawget(_G, "FIREWORKS_COMET_GIF_CAPTURE_PROBE") == PROBE then
    _G.FIREWORKS_COMET_GIF_CAPTURE_PROBE = nil
  end
end
...
if not (PROBE.report.loaded and tmr_mod and tmr_mod.create) then
  ...
  local function stage_comet()
    local state = target.state
    for _, rocket in ipairs(state.rockets) do
      if rocket.kind == COMET_KIND then
        rocket.x = 160
        rocket.y = 222
        rocket.vx = 0.18
        rocket.vy = -4.90
  ...
  local function capture_frame(frame)
    ...
    local path = OUTPUT_DIR .. "/comet-" .. label .. ".png"
    ...
    if saved_ok then
      PROBE.report.captures.comet = PROBE.report.captures.comet + 1
    end
  ...
  local function finish()
    ...
    PROBE.report.ok = PROBE.report.version_matches == true
      ...
      and PROBE.report.captures.comet == FRAME_COUNT
  ...
  local function wait_for_comet(attempt)
    if stage_comet() then
      schedule(80, function() capture_sequence(1) end)
    elseif attempt >= 25 then
      PROBE.report.fatal = "comet rocket did not spawn"
      write_report()
      stop_and_exit("spawn-failed")
    else
      schedule(20, function() wait_for_comet(attempt + 1) end)
    end
  end
